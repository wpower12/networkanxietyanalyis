...
ANX_THRESHOLDS = [i * 0.005 for i in range(10)]
...
df_huts = pd.read_csv(FN_OUT_RAW,
...
results = []
for anx_threshold in ANX_THRESHOLDS:
    print("processing threshold: {}".format(anx_threshold))
    total_events = 0
    total_examples = 0
    for u, user in enumerate(tqdm(users)):
    ...
    results.append((anx_threshold, total_events, total_examples))


print("threshold, events, examples")
for (a_thresh, n_events, n_examples) in results:
    print("{}, {}, {}".format(a_thresh, n_events, n_examples))
